...
struct param::impl {
  ...
  impl(std::string tree1, std::string tree2, int param, bool verbose)
      : _s(tree1), _t(tree2), _param(param), _verbose(verbose) {
  ...
  void formulate() {
    ...
    _param_row = count;
    glp_set_row_name(mip, count, "P");
    glp_set_row_bnds(mip, count, GLP_UP, 0.0, _param);
  ...
  std::pair<int, int> solve(int param) {

    glp_set_row_name(mip, _param_row, "P");
    glp_set_row_bnds(mip, _param_row, GLP_UP, 0.0, param);
    ...
    return std::make_pair(opt, glp_get_prim_stat(mip));
  }
};

param::param(std::string tree1, std::string tree2, int param, bool verbose) :
    _internals(std::make_shared<impl>(tree1, tree2, param, verbose)) {}

void param::formulate() {
  _internals->formulate();
}

int param::solve(int param) {
  auto solve_pair = _internals->solve(param);
  return solve_pair.first;
}

void param::delete_problem() {
